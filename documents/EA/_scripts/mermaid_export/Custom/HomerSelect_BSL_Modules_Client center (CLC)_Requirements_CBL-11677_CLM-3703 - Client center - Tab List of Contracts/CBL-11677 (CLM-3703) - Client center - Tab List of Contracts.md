# CBL-11677 (CLM-3703) - Client center - Tab List of Contracts

- **Diagram Type**: Custom
- **Package**: HomerSelect/BSL/Modules/Client center (CLC)/Requirements/CBL-11677/CLM-3703 - Client center - Tab List of Contracts
- **Diagram ID**: 156161
- **Elements**: 5
- **Connectors**: 1

```mermaid
graph TD
    CLM_3703_Client_center_Tab_List_of_Contracts["CLM-3703 Client center - Tab List of Contracts"]
    Contract_Search_Contract_Search["Contract/Search : Contract/Search"]
    User_Interface_Model_Tab_List_of_contracts["User Interface Model : Tab List of contracts"]
    ADD_Show_tab_List_of_contracts["{ADD}Show tab List of contracts"]
    MOD_06_030_Show_Client_Data["{MOD}06.030 Show Client Data"]
    MOD_06_030_Show_Client_Data -->|unnamed| ADD_Show_tab_List_of_contracts
```
