---
type: Requirement
stereotype: "Access Right"
package: "HomerSelect/BSL/Requirements Model/Finished/CLM/CBL-6279 (CLM-2061) New access right for Client Documents"
domain: "Requirements Model"
element_id: 1638528
diagrams: 5
connections: 1
tags:
  - requirement
  - requirements-model
---

# 📋 {MOD}01.128 Preview document (all statuses)

> **Type**: Requirement · **Stereotype**: «Access Right»
> **Package**: HomerSelect/BSL/Requirements Model/Finished/CLM/CBL-6279 (CLM-2061) New access right for Client Documents

## 📝 Notes

Specific right for UC activation.

	
- Code = UC01_128_AllContractStatuses


This access right is used with rule Access control to Document by Contract status - user can access to {DEL CLM-2575}{ADD CLM-2061) client or contract{/ADD}{/DEL} documents without any contract-status restrictions.

## 🔗 Connections (1)

- ← Realisation: [[{MOD}01.128 Preview document]]

## 📊 Appears In (5 diagrams)

- Custom: CBL-6279 (CLM-2061) New access right for Client Documents
- Custom: CBL-8361 (CLM-2575) Optimize access rights for documents download on affected tabs
- Custom: Client documents - Access Rights
- Custom: COMMON for documents
- Custom: Contract documents - Access Rights
