---
type: Requirement
stereotype: "Functional"
package: "HomerSelect/BSL/Requirements Model/Finished/CLM/CBL-8394 (CLM-2583)  Extend contract service information window and add service tooltip"
domain: "Requirements Model"
element_id: 1474291
diagrams: 1
connections: 0
tags:
  - requirement
  - requirements-model
---

# 📋 REQ #2 - Add label Contract Service.Service version to Service information modal window

> **Type**: Requirement · **Stereotype**: «Functional»
> **Package**: HomerSelect/BSL/Requirements Model/Finished/CLM/CBL-8394 (CLM-2583)  Extend contract service information window and add service tooltip

## 📝 Notes

Goal of this requirement is to add label Contract Service.Service version to Service information modal window.

## 📊 Appears In (1 diagrams)

- Custom: CBL-8394 (CLM-2583) Extend contract service information window and add service tooltip
