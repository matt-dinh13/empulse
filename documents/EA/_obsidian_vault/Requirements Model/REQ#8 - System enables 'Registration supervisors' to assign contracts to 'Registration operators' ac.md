---
type: Requirement
stereotype: "FunctionalRequirement"
package: "HomerSelect/BSL/Requirements Model/Finished/CLM/CLM-865 (CBL-1142) IN Paperless REQ10 - Contract registration process"
domain: "Requirements Model"
element_id: 1272121
diagrams: 1
connections: 2
tags:
  - requirement
  - requirements-model
---

# 📋 REQ#8 - System enables 'Registration supervisors' to assign contracts to 'Registration operators' according to opened registration mistake.

> **Type**: Requirement · **Stereotype**: «FunctionalRequirement»
> **Package**: HomerSelect/BSL/Requirements Model/Finished/CLM/CLM-865 (CBL-1142) IN Paperless REQ10 - Contract registration process

## 🔗 Connections (1)

- → Generalization: [[Modify the current Registration queue to usage by IN business as follow_ - definition of business ru]]

## 📊 Appears In (1 diagrams)

- Custom: CLM-865 (CBL-1142) IN Paperless REQ10 - Contract registration process
