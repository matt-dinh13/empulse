...
{ADD CLM-2381 /} -- taken from the original LOR function Get active offers of the party; created for CLM usage only!
...
- If the FilterOffersAvailableOnSalesroom = TRUE and SalesroomCode was provided on the input, then system takes the offers received in previous step and checks their availability on the corresponding salesroom according to the rule {ADD}Check Customer offers availability on Salesroom with following parameters:
...
- → Dependency: [[{DEL}Check Customer offers availability on Salesroom]]
