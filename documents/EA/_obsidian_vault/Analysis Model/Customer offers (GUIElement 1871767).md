...
type: GUIElement
stereotype: ""
package: "HomerSelect/BSL/Analysis Model/Contract Origination/Application detail/User Interface Model"
...
- Contract is in status {DEL DFPH-245}Approved ('S'){/DEL} or Canceled ('T').
...
There is no active scoring of respective customer in progress (or such scoring has not been triggered based on the underwriting decision done during the evaluation of actual application), which means:
   - Contract->External_Identifier[.Key = 'RECALC_SCORING_ID'] does not exist.
      or
   - AND Contract->External_Identifier[.Key = 'RECALC_SCORING_ID'] exists AND the scoring with respective identifier has already finished (i.e. RBP_Scoring[.Request_ID=External_Identifier.Value].Result is not null).
