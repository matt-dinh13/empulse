...
Related to Mobile phone 2

LDM Source:
LDM Source: Temporary_Application.TempAppl_Contact.Contact_is_ verified where Temporary_Application.TempAppl_Contact.Type = SECONDARY.For developers: In DB, the entity is represented by table TA_CONTACT, records are distinguished by columns CONTACT_TYPE_CODE and CONTACT_ROLE_TYPE (= "CL").

Editable:	

	
- 2nd BoD 2SP - only if Mobile phone 2 is not empty
