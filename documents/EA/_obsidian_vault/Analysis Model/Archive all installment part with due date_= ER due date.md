---
type: Action
stereotype: ""
package: "HomerSelect/BSL/Analysis Model/Contract Management/Loan Service Processing/Loan Option CEL/COMMON for Early Repayment/Use Case model"
domain: "Analysis Model"
element_id: 1876345
diagrams: 1
connections: 2
tags:
  - action
  - analysis-model
---

# 📄 Archive all installment part with due date>= ER due date

> **Type**: Action
> **Package**: HomerSelect/BSL/Analysis Model/Contract Management/Loan Service Processing/Loan Option CEL/COMMON for Early Repayment/Use Case model

## 🔗 Connections (2)

- ← ControlFlow: [[03.070 Calculate early repayment amount (Activity 1876347)]]
- → ControlFlow: [[Get maximum number of active standard installments]]

## 📊 Appears In (1 diagrams)

- Activity: 03.110 Perform early repayment installment schedule
