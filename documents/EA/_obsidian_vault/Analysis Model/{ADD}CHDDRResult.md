---
type: Class
stereotype: ""
package: "HomerSelect/BSL/Analysis Model/Contract Management/Customer Self-Care API/Application Interface Model/CSI OpenAPI/Contract Service Requests/Contract CHDDR Service Requests"
domain: "Analysis Model"
element_id: 1561102
diagrams: 2
connections: 4
tags:
  - class
  - analysis-model
---

# 🔷 {ADD}CHDDRResult

> **Type**: Class
> **Package**: HomerSelect/BSL/Analysis Model/Contract Management/Customer Self-Care API/Application Interface Model/CSI OpenAPI/Contract Service Requests/Contract CHDDR Service Requests

## 📝 Notes

{ADD CLM-1856 /}
A preview of PER service request data related to provided contract and date

## 🔗 Connections (4)

- ← Dependency: [[{ADD}CHDDRServiceRequestPreview]]
- → Dependency: [[{ADD}CHDDRInstallmentSelection]]
- → Dependency: [[{ADD}CHDDRDueDaySelection]]
- ← Dependency: [[{ADD}CHDDRServiceRequest]]

## 📊 Appears In (2 diagrams)

- Logical: Contract CHDDR Service Requests - create request
- Logical: Contract CHDDR Service Requests - get preview

## 📋 Attributes

| Name | Type | Default |
|------|------|---------|
| installmentSelection | CHDDRInstallmentSelection |  |
| dueDaySelection | CHDDRDueDaySelection |  |
| fee | money |  |
