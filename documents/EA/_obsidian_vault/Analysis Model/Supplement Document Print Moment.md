---
type: Class
stereotype: ""
package: "HomerSelect/BSL/Analysis Model/Contract Supplements/COMMON for Supplements/Supplement definition/Logical Data Model"
domain: "Analysis Model"
element_id: 1879275
diagrams: 5
connections: 2
tags:
  - class
  - analysis-model
---

# 🔷 Supplement Document Print Moment

> **Type**: Class
> **Package**: HomerSelect/BSL/Analysis Model/Contract Supplements/COMMON for Supplements/Supplement definition/Logical Data Model

## 📝 Notes

There is defined moment when (and hereinafter) a document printout based on a particular document type is to be created.

## 🔗 Connections (2)

- → Association: [[{MOD}Check Moment Type]]
- → Aggregation: [[Supplement Document Type (Class 1879273)]]

## 📊 Appears In (5 diagrams)

- Logical: Supplement Definition
- Logical: Supplement definition domain - Logical data model
- Logical: Supplement definition for Collection tool service
- Logical: Supplement definition for Credit limit change
- Logical: Supplement definition for Loan restructuring
