...
# 📋 Standard priority pairing method for particular T&C
...
Note: This algorithm is a copy of Standard priority pairing method alg. The only one change is that Terms and conditions are passed on the input, so pairing priority is performed according to passed T&C, not to current T&C. 
...
The Priority is chosen according to {DELL CBL-15541 IS-1558}Condition Change Flag{DELL} {ADD CBL-15541 IS-1558}Condition Flag{/ADD} parameter which divides installments to "before due date" and "overdue". In case child installment is related to a parent Installment, Due date of the parent Installment is taken, else the child Installment Due date is used
