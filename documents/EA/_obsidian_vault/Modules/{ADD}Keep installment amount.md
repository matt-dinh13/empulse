---
type: GUIElement
stereotype: "lcheckbox"
package: "HomerSelect/BSL/Modules/Product Catalog (PRC)/Analytical Model/Service/User Interface for Service Management/Service Type Specific Extension/LRES/User Interface"
domain: "Modules"
element_id: 1530468
diagrams: 1
connections: 0
tags:
  - guielement
  - modules
---

# 🖥️ {ADD}Keep installment amount

> **Type**: GUIElement · **Stereotype**: «lcheckbox»
> **Package**: HomerSelect/BSL/Modules/Product Catalog (PRC)/Analytical Model/Service/User Interface for Service Management/Service Type Specific Extension/LRES/User Interface

## 📝 Notes

{ADD PCG-2383/}
LDM: SERVICE.LoanRestructuringService.KeepInstallmentAmount

Localization Code: SER_LRES_KeepInstallmentAmount

## 📊 Appears In (1 diagrams)

- Custom: Set Loan Restructuring properties
