...
# 📋 DPD priority pairing method for particular T&C
...
Note: This algorithm is a copy of DPD priority pairing method alg. The only one change is that Terms and conditions are passed on the input, so pairing priority is performed according to passed T&C, not to current T&C. 
