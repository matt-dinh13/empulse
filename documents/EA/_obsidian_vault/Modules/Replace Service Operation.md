---
type: Class
stereotype: ""
package: "HomerSelect/BSL/Modules/Contract bulk operations (BSA)/Analytical Model/COMMON for Contract Bulk Operations/Logical Data Model"
domain: "Modules"
element_id: 1877968
diagrams: 2
connections: 1
tags:
  - class
  - modules
---

# 🔷 Replace Service Operation

> **Type**: Class
> **Package**: HomerSelect/BSL/Modules/Contract bulk operations (BSA)/Analytical Model/COMMON for Contract Bulk Operations/Logical Data Model

## 🔗 Connections (1)

- → Generalization: [[Bulk Operation]]

## 📊 Appears In (2 diagrams)

- Logical: COMMON Logical Data Model
- Logical: Logical Data Model

## 📋 Attributes

| Name | Type | Default |
|------|------|---------|
| New Service Code | string |  |
| New Service Version | int |  |
| Original Service Code | string |  |
| Original Service Version | int |  |
| Replace Lower Version | boolean |  |
